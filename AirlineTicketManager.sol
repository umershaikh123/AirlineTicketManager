...
uint FirstClassPrice;
uint BusinessClassPrice;
uint EconomyClassPrice;
...
event UserRegistered(string _name , string  _destination , uint _passportId , address addr);
...
function RegisterUser(string memory _name , string memory _destination , uint _passportId) external CheckUser {
...
function BookFirstClassTicket() external payable onlyWhitelisted OnlyOneTicket(msg.sender) {
    require(msg.value >=  FirstClassPrice  , "Not Enough Ether for First Class Ticket");
...
function BookBusinessClassTicket() external payable onlyWhitelisted  OnlyOneTicket(msg.sender) {
    require(msg.value >= BusinessClassPrice , "Not Enough Ether for Business Class Ticket");
    address payable extra = payable(msg.sender);
...
function BookEconomyClassTicket() external payable onlyWhitelisted  OnlyOneTicket(msg.sender) {
    require(msg.value >= EconomyClassPrice , "Not Enough Ether for Economy Ticket");
    address payable extra = payable(msg.sender);
...
function setFirstClassPrice(uint newPrice) external  onlyOwner returns (uint)  {
    FirstClassPrice = newPrice;
    return FirstClassPrice;
}

function setBusinesstClassPrice(uint newPrice) external  onlyOwner returns (uint)  {
    BusinessClassPrice = newPrice;
    return BusinessClassPrice;
}


function setEconomyClassPrice(uint newPrice) external  onlyOwner returns (uint)  {
